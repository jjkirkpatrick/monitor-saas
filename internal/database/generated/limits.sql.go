...
const checkUserLimits = `-- name: CheckUserLimits :one
...
type CheckUserLimitsRow struct {
	UserID                      uuid.UUID
	MaxMonitors                 int32
	...
	MaxAlertCountPerMonitor     int64
	CurrentNotificationChannels int32
	LimitStatus                 string
}

func (q *Queries) CheckUserLimits(ctx context.Context, userID uuid.UUID) (CheckUserLimitsRow, error) {
	row := q.db.QueryRow(ctx, checkUserLimits, userID)
	var i CheckUserLimitsRow
	err := row.Scan(
		&i.UserID,
		&i.MaxMonitors,
		&i.MaxChecksPerDay,
		...
		&i.CurrentCheckCount,
		&i.MaxAlertCountPerMonitor,
		&i.CurrentNotificationChannels,
		&i.LimitStatus,
	)
	return i, err
}

const getUserLimitsWithUsage = `-- name: GetUserLimitsWithUsage :one
...
type GetUserLimitsWithUsageRow struct {
	UserID                    uuid.UUID
	MaxMonitors               int32
	MaxChecksPerDay           int32
	MaxAlertConfigsPerMonitor int32
	...
	CreatedAt                 pgtype.Timestamptz
	UpdatedAt                 pgtype.Timestamptz
	TotalChecks30d            int64
	TotalAlerts30d            int64
	TotalNotifications30d     int64
	TotalApiCalls30d          int64
	TotalDataTransfer30d      int64
}

func (q *Queries) GetUserLimitsWithUsage(ctx context.Context, userID uuid.UUID) (GetUserLimitsWithUsageRow, error) {
	row := q.db.QueryRow(ctx, getUserLimitsWithUsage, userID)
	var i GetUserLimitsWithUsageRow
	err := row.Scan(
		&i.UserID,
		&i.MaxMonitors,
		&i.MaxChecksPerDay,
		...
		&i.TotalChecks30d,
		&i.TotalAlerts30d,
		&i.TotalNotifications30d,
		&i.TotalApiCalls30d,
		&i.TotalDataTransfer30d,
	)
	return i, err
}
...
func (q *Queries) GetUserUsageByDateRange(ctx context.Context, arg GetUserUsageByDateRangeParams) ([]GetUserUsageByDateRangeRow, error) {
	rows, err := q.db.Query(ctx, getUserUsageByDateRange, arg.UserID, arg.Date, arg.Date_2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const incrementUsageStats = `-- name: IncrementUsageStats :exec
...
type IncrementUsageStatsParams struct {
	UserID                 uuid.UUID
	TotalChecks            int32
	TotalAlertsTriggered   int32
	TotalNotificationsSent int32
...
func (q *Queries) IncrementUsageStats(ctx context.Context, arg IncrementUsageStatsParams) error {
	_, err := q.db.Exec(ctx, incrementUsageStats,
		arg.UserID,
		arg.TotalChecks,
